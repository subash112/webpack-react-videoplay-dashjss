...
const List = (props) => {
  ...
  const [MovieUnits, setMovieUnits] = useState(
  ...
  useEffect(() => {
    instance.get(`v3/lists/${props.id}`).then((response) => {
  ...
  const PrepareData = (movie, lowRange, highRange) => {
    ...
    if (lowRange === undefined && highRange === undefined) {
      LowRanges = 0;
      HighRanges =
        movie.contents.data.length > MovieUnits
          ? MovieUnits
          : movie.contents.data.length;
    } else {
      (LowRanges = lowRange), (HighRanges = highRange);
    }
      ...
      {Movie !== undefined && (
        <>
          ...
          {HighRange !== Movie.contents.data.length && (
            <div
              className='arrow right float-right cursor-pointer'
              onClick={() => {
                let highRange =
                  HighRange + MovieUnits > Movie.contents.data.length
                    ? Movie.contents.data.length
                    : HighRange + MovieUnits;
          ...
          {LowRange !== 0 && (
            <div
              className='arrow left float-left cursor-pointer arrow-position'
              onClick={() => {
                let highRange = LowRange;
                let lowRange =
                  LowRange - MovieUnits > 0 ? LowRange - MovieUnits : 0;
